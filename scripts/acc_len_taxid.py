...
def prepare_files(args, tmp_output_folder, use_assembly, path_exec, input_files, input_files_from_directory):
    ...
    if args.seq_info_file: # file already provided 
        ...
        taxsbp_input_file = retrieve_ncbi(args.db_prefix, tmp_output_folder, input_files, input_files_from_directory, args.threads, path_exec, args.seq_info, use_assembly)
...
def get_taxdump(tmp_output_folder):
    ...
    run_wget_taxdump_cmd = 'wget -qO {0} "https://ftp.ncbi.nih.gov/pub/taxonomy/taxdump.tar.gz"'.format(taxdump_file)
...
def get_accession2taxid(acc2txid, tmp_output_folder):
    ...
    run_wget_acc2txid_file_cmd = 'wget -qO {0} "https://ftp.ncbi.nih.gov/pub/taxonomy/accession2taxid/{1}.accession2taxid.gz"'.format(acc2txid_file, acc2txid)
    stdout, stderr = run(run_wget_acc2txid_file_cmd, print_stderr=True)
    print_log("Done. Elapsed time: " + str("%.2f" % (time.time() - tx)) + " seconds.\n")
    return acc2txid_file

def retrieve_ncbi(db_prefix, tmp_output_folder, input_files, input_files_from_directory, threads, path_exec, seq_info, use_assembly):

    taxsbp_input_file = db_prefix + '_acc_len_taxid.txt'
